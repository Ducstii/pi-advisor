...
const jiti = createJiti(import.meta.url, {
  ...
  alias: {
    "@earendil-works/pi-coding-agent": fileURLToPath(
      new URL("./pi-stub.mjs", import.meta.url),
    ),
    "@earendil-works/pi-tui": fileURLToPath(
      new URL("./tui-stub.mjs", import.meta.url),
    ),
    typebox: new URL("../node_modules/typebox/build/index.mjs", import.meta.url)
      .pathname,
  },
});
const tools = [];
...
const throws = async (fn, re, l) => {
  try {
    ...
    ok(re.test(e.message), `${l} → ${e.message.slice(0, 60)}`);
  }
};
      ...
      questions: [
        {
          id: "c",
          type: "noul",
          instructions: "How confident are you in this",
        },
      ],
    }),
  /do not ask Jev for its own confidence/,
  "confidence question rejected",
);
...
ok(
  /q1 → a \(confidence 0\.80\) · next: b 0\.35 · c 0\.25 · margin 0\.05/.test(
    text,
  ),
  "margin + third option shown → " + text.split("\n")[1],
);
ok(
  /margin 0\.05/.test(text) && !/NEAR-TIE/.test(text.split("\n")[1]),
  "0.05 margin no longer trips NEAR-TIE (margin <= delta only)",
);
...
globalThis.fetch = origFetch;
ok(
  /NEAR-TIE/.test(r2.content[0].text),
  "confidence exactly 0.60 flags NEAR-TIE",
);
